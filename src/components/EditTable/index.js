...
import { renderElement, createFormatter } from "../util";
import "./style";
...
class EditCell extends UIComponent {
  ...
  render() {
    const { cell, config, column, rowIndex, columnIndex } = this.props;
    const { value, cellProps } = cell;
    // 支持行按钮，行按钮一直显示
    return (
      <div className="editable-cell">
        {config.isCellEditable(rowIndex, columnIndex) ||
        ["buttons", "buttongroup", "toolbar"].indexOf(column.type) > -1 ? (
            ...
            {this.renderItem(column, {
              ...
              onChange: (value) =>
                this.context.onEvent(
                  ...
                  "setCell",
                  value,
                  async () => await config.setCell(rowIndex, columnIndex, value)
                ),
              onPressEnter: () =>
                this.context.onEvent(
                  config,
          ...
          <div
            ...
            onClick={() =>
              this.context.onEvent(
                ...
                "startEdit",
                undefined,
                async () => await config.startEdit(rowIndex, columnIndex)
              )
            }
          >
...
export default class EditTable extends UIComponent {
  ...
  numColumn = {
    ...
    render: (text, row) => {
      ...
      if (row.error) {
        errmsg = (
          <Tooltip title={row.error} overlayClassName="errortip">
  ...
  render() {
    ...
    return (
      <div className={`${style.prefix}-editableTable`}>
        {btns}
        <Table
          bordered={bordered}
          ...
          onHeaderRow={(column, index) => ({
            onClick: () => {
              this.context.onEvent(
                ...
                "selectColumn",
                { column, index },
                () => config.selectColumn(index)
              );
            },
          })}
          rowSelection={rowSelection}
          rowClassName={(row, index) =>
            rowIndex === index ? "row-selected" : ""
          }
          onRow={(row, index) => ({
            onClick: () => {
              this.context.onEvent(
