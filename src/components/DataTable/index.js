...
export default class Table extends UIComponent {
  renderHeaderCell = ({ columnIndex, key, rowIndex, style }) => {
    const { title } = this.props.config.getColumn(rowIndex, columnIndex);
    return (
  ...
  renderBodyCell = ({ columnIndex, key, rowIndex, style }) => {
    const { value, type = "" } = this.props.config.getCell(
      rowIndex,
      columnIndex
    );
    return (
      <a
        className={["cell", type].join(" ")}
        key={key}
        style={style}
        onClick={() => this.props.config.selectCell(rowIndex, columnIndex)}
      >
        {value}
      </a>
    );
  ...
  render() {
    ...
    return (
      <div className={`${style.prefix}-datatable${cellMeasurer ? " measurer" : ""}`}>
        <ScrollSync>
            ...
            return (
              <InfiniteLoader
                ...
                loadMoreRows={({ startIndex, stopIndex }) =>
                  config.loadData(startIndex, stopIndex - startIndex)
                }
                rowCount={rowCount}
              >
