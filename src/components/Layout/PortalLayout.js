...
export default class PortalLayout extends React.Component {
  ...
  render() {
    const { layout, items = [] } = this.props.config;
    let hasFocus = false;
    let contents = [];
    for (let i = 0, l = items.length; i < l; i++) {
      ...
      if (layout === "topbottom") {
        if (i === 0) {
          contents.push(
            <Header className="header" key={it.key}>
          ...
          contents.push(
            <Content className="content" key={it.key}>
              {this.props.renderItem(it, { autoFocus }, items)}
            </Content>
          );
        } else if (i === 2) {
          contents.push(
            <Footer className="footer" key={it.key}>
              {this.props.renderItem(it, { autoFocus }, items)}
            </Footer>
          );
        } else {
      ...
      if (layout === "sidercontent") {
        if (i === 0) {
          contents.push(
            <Sider className="sider" key={it.key}>
              {this.props.renderItem(
                it,
                { autoFocus, type: "menu", mode: "inline" },
                items
              )}
            </Sider>
          );
    ...
    return (
      <Layout className={[`${style.prefix}-layout`, "portal"].join(" ")}>
        {contents}
      </Layout>
    );
  }
